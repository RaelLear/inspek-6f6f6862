  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <section className="mx-auto max-w-4xl px-4 py-16 text-center md:py-24">
        <div className="inline-block rounded-full bg-destructive/10 px-4 py-1 text-sm font-bold text-destructive mb-6">
          🔥 Para profissionais de SESMT
        </div>
      ...
      <section className="border-t bg-muted/30 py-16">
        <div className="mx-auto max-w-4xl px-4">
          ...
          <div className="mt-10 grid gap-6 md:grid-cols-3">
            {[
              { icon: Clock, title: 'Inspeção em segundos', desc: 'Código + ponto + conformidade. Tudo no celular, sem papel.' },
      ...
      <section className="py-16">
        <div className="mx-auto max-w-4xl px-4">
          ...
          <div className="mt-10 space-y-6">
            {[
              { step: '1', title: 'Cadastre seus extintores', desc: 'Código, ponto, tipo, peso e datas. Uma vez e pronto.' },
              { step: '2', title: 'Faça inspeções no celular', desc: 'Navegue pelo fluxo guiado: código → ponto → conformidade → datas. Tudo salvo na nuvem.' },
      ...
      <section className="py-16">
        <div className="mx-auto max-w-2xl px-4">
          <div className="rounded-2xl border-2 border-foreground bg-card p-8 text-center">
            <h2 className="text-3xl font-black">Acesso Completo</h2>
            <div className="mt-6 space-y-3 text-left">
              {[
                'Plataforma completa de gestão de extintores',
                'Inspeções ilimitadas no celular',
                'Dashboard com métricas e relatórios',
                'Alertas automáticos de vencimento',
                'Planilha para impressão',
                'Ambientes compartilhados (equipes)',
                'Atendimento via WhatsApp',
              ].map((item, i) => (
                <div key={i} className="flex items-center gap-2">
                  <CheckCircle className="h-5 w-5 shrink-0 text-status-approved" />
                  <span className="text-sm font-medium">{item}</span>
                </div>
              ))}
            </div>
            <div className="mt-8">
              <p className="text-muted-foreground text-sm line-through">De R$ 17/mês</p>
              <p className="text-5xl font-black">R$ 10<span className="text-lg font-bold text-muted-foreground">/mês</span></p>
              <p className="text-sm font-bold text-status-approved mt-1">Você economiza R$7,00</p>
            </div>
            <div className="mt-4 flex flex-col gap-3">
              <Button size="lg" className="w-full h-14 text-lg font-bold gap-2" onClick={() => window.open(CHECKOUT_URL, '_blank')}>
                <ArrowRight className="h-5 w-5" /> Quero meu acesso
              </Button>
            </div>
          </div>
        </div>
      ...
      <footer className="border-t py-8 text-center text-sm text-muted-foreground">
        <div className="mx-auto max-w-4xl px-4">
          ...
          <p>© {new Date().getFullYear()} inSpek. Todos os direitos reservados.</p>
        </div>
      </footer>
    </div>
  );
